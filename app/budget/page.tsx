...
export default function Budget() {
  ...
  const initialItems = React.useMemo(() => {
    if (income === 0) return [];
    return [
      { id: "housing", name: "Housing", bucket: "Needs" as Bucket, amount: Math.round(income * 0.3) },
      { id: "food", name: "Food", bucket: "Needs" as Bucket, amount: Math.round(income * 0.1) },
      { id: "transport", name: "Transport", bucket: "Needs" as Bucket, amount: Math.round(income * 0.05) },
      { id: "utilities", name: "Utilities", bucket: "Needs" as Bucket, amount: Math.round(income * 0.05) },
      { id: "entertainment", name: "Entertainment", bucket: "Wants" as Bucket, amount: Math.round(income * 0.15) },
      { id: "subscriptions", name: "Subscriptions", bucket: "Wants" as Bucket, amount: Math.round(income * 0.05) },
      { id: "shopping", name: "Shopping", bucket: "Wants" as Bucket, amount: Math.round(income * 0.1) },
      { id: "emergency", name: "Emergency savings", bucket: "Savings" as Bucket, amount: Math.round(income * 0.15) },
      { id: "investing", name: "Investing", bucket: "Savings" as Bucket, amount: Math.round(income * 0.05) },
    ];
  }, [income]);
          ...
          <Card className="rounded-2xl bg-slate-900/80 border-slate-800/70 shadow-lg">
            ...
            <CardContent className="space-y-5">
              <p className="text-sm text-slate-300">
                This page builds a standard 50/30/20 budget from the income in your profile.
                Edit line items below or add new ones to match reality. When you save, totals
                (expenses & savings) will be updated in your profile.
              </p>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-xl border border-white/10 bg-slate-800/60 p-3">
                  <div className="text-xs text-slate-400">Targets (50/30/20)</div>
              ...
              {overages.length > 0 && (
                <WarningBanner
                  messages={overages.map(
                    ({ b, over }) =>
                      `${b} is ${formatCurrency(Math.ceil(over))} over its target. Try readjusting your budget.`
                  )}
                />
              )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {items.map((it) => (
                    ...
                    >
                      <div className="w-40 text-sm text-slate-200">{it.name}</div>

                      <select
                        value={it.bucket}
                        onChange={(e) =>
                          updateItem(it.id, { bucket: e.target.value as Bucket })
                        }
              ...
              <div className="pt-4 border-t border-white/10">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <Button
                    onClick={() => {
                      if (!newName || !newAmount) return alert("Provide name and amount");
          ...
          <Card className="rounded-2xl bg-slate-900/80 border-slate-800/70 shadow-lg">
            ...
            <CardContent>
              ...
              <div className="space-y-2 text-sm">
                <Row
                  label="Needs"
                  value={`${formatCurrency(totals.Needs)} / ${formatCurrency(targets.Needs)}`}
                  dotClass="bg-blue-500"
                  over={isOver("Needs")}
                />
                <Row
                  label="Wants"
                  value={`${formatCurrency(totals.Wants)} / ${formatCurrency(targets.Wants)}`}
                  dotClass="bg-orange-500"
                  over={isOver("Wants")}
                />
                <Row
                  label="Savings"
                  value={`${formatCurrency(totals.Savings)} / ${formatCurrency(targets.Savings)}`}
...
function Chip({
  ...
  return (
    <span
      className={`rounded-md px-2 py-1 text-xs font-medium border ${over ? danger : neutral}`}
    >
      {label}
    </span>
  );
...
function Row({
  ...
  return (
    <div className={`flex items-center justify-between rounded-md border ${border} ${bg} px-3 py-2`}>
